...
human_scores <- preprocessed_experiment_df |>
  ...
  ungroup() |>
  select(prolific_id, teacher_condition, block_condition, block_index, game_type, condition_index, condition_count_index, total_reward)
...
model_scores <- read_csv(here("data/experiment_experiment2_solver_Thompson_network_arch_fc_hiddenDim_1024_oneHot_True_numArms_12_obsWindow_24", "model_performance.csv"))
model_scores
...
temp1 <- human_scores |>
  filter(game_type!='watch') |>
  arrange(prolific_id, block_condition, teacher_condition, condition_index, game_type, block_condition, condition_count_index)
temp1
...
cor.test(temp1$total_reward, temp2$total_reward)

cor.test(temp1$total_reward, temp3$total_reward)

cor.test(temp1$total_reward, temp4$total_reward)

#rmse <- sqrt(mean((temp1$total_reward - temp2$total_reward)^2))
#rmse
```